...
现在增加显式`--first-pass`入口：在已完成normalize-v2的新题中，自动纳入全部
符合合同的dependency失败，每题只使用checked-v2回修一次，不先跑旧v1。
审核与前提检查没有放宽；不允许携带旧v1历史、从另一个回修继续回修，或混淆
`checked_first_pass`和`development_iteration=2`。旧开发入口保持兼容。
该入口已经通过合成响应的端到端、离线重放、不重试与来源篡改测试。

normalize入口的`--development-run`绑定原开发题，按题目/代码/源输出逐项比较，
从新构图批次排除整个开发集（接受和失败均保留原结果），不择优重跑。
开发题仍包含在全量CPU执行分母中；其是否可纳入最终接受集还要看全量复验。
生产配置`livecodebench-calibri-full-normalize.json`对应87道新题、最多261次请求、
6 workers、32k输出。22265390预留token是总额2500万扣除历史2734610后的余额，
不是另加额度；回修只能从首轮实际剩余额度中分配，不并行重复分配两份余额。

CPU作业111489在准备阶段因旧tar中的AppleDouble元数据使源码清单不匹配而失败，
没有执行候选。保留原运行；111490在新目录中仅排除元数据，真实源码哈希与旧冻结
提交完全一致，以原资源复验成功结束。93候选，91全部测试通过，2题触发SIGXCPU，
不能把超时写作错误答案。没有修改测试、放宽限额或覆盖失败运行。
准备脚本现在更早检查快照，避免发现打包错误之前生成大体积测试输入。
新增`audit_calibri_full_execution.py`从固定原始v6独立重建输入，避免把远端返回的
input-manifest误称为本地预提交清单；不执行候选程序、不向API发送隐藏测试。

### 首轮暂停：审核输出缺字段（2026-09-23）

冻结提交`fab6b19`的新87题运行未完成。17题有终态：3模型接受、2语义拒绝、
6依赖结构失败、6审核schema失败。后者均为完整stop响应，但缺少必填的
`checks.no_invariant_assumed`；该项在提示和校验器中均被要求。
六份原始decision为5 accept、1 reject，均未因缺字段而获自动接受；原始负面裁决保留。
这是重复的输出合同问题，不足以认定候选图错误，也不允许自动填true或降低验收规则。

运行器缺少外部停止派发/排空接口，因此核对精确worker进程组后停止该批；
6个在途请求未保存响应，远端结果/计费未知，全额预留并保留请求，不自动重放。
57请求、51响应、3823239预留token；含此前历史为96请求、6557849预留token，
总上限仍600请求/2500万。没有正常completion，不把此批称为完整首轮或估计最终通过率。
旧开发3接受与新题3接受均仍为模型审核候选，未正式发布。

本地自动跟进已暂停，没有B1/GPU操作。后续建议是加强完整字段模板、增加停止排空
和阶段续接测试，并为无负面裁决的固定图限定一次完整补审；补审/未知请求恢复协议
尚待批准，不在本记录中承诺或启动。原始题目、响应、凭据与停止明细仅保留私有目录。
