...
def probe_contract(root, config, client, *, configured_max_tokens=False):
    ...
    if type(configured_max_tokens) is not bool:
        raise ValueError("configured_max_tokens must be boolean")
    if configured_max_tokens and config.max_tokens > 32768:
        raise ValueError("configured-cap probes are bounded to 32768 output tokens")
    probe_cap = config.max_tokens if configured_max_tokens else min(
        config.max_tokens, 4096 if config.thinking == "enabled" else 64
    )
    token_budget = 2 * probe_cap + 32768 if configured_max_tokens else 32768
    config = replace(
        config,
        workers=1,
        rate_limit_retries=0,
        max_calls=min(config.max_calls, 2),
        max_reserved_tokens=min(config.max_reserved_tokens, token_budget),
        strict_response_contract=True,
        content_gated_response=False,  # Diagnostic failures must remain failures.
    )
    if config.thinking not in ("enabled", "disabled"):
        raise ValueError("this probe requires an explicit thinking profile")
    ...
    with run_lock(root):
        ...
        write_once(runner.root / "probe_policy.json", {
            "configured_max_tokens": configured_max_tokens,
            "actual_request_max_tokens": probe_cap,
            "max_calls": config.max_calls,
            "max_accounted_tokens": config.max_reserved_tokens,
        })
        runner._restore_budget()
        results = []
        last_integer = 19999 if configured_max_tokens else 9999
        for name, instruction in (
            ("short", 'Return exactly this JSON object: {"ok":true}'),
            ("cap", f'Return a JSON object with key "numbers" containing an array of every integer from 0 through {last_integer}. Write all integers; no ranges, ellipses, formulas or explanations.'),
        ):
            ...
            # Thinking needs its own output allowance, including reasoning.
            # Keep the cap probe bounded, but do not require a thinking model
            # to finish even a trivial response in the legacy 64-token budget.
            request["max_tokens"] = probe_cap
